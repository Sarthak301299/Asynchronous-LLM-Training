...
class COLATokenizedDataset(Dataset):
...
def main():
    ...
    #Get Dataset online
    #train_dataset = load_dataset("nyu-mll/glue","cola", split='train')
    #val_dataset = load_dataset("nyu-mll/glue","cola", split='validation')
    #test_dataset = load_dataset("nyu-mll/glue","cola", split='test')
    ...
    #tokenized_val_dataset = val_dataset.map(tokenize_function, batched=True, num_proc=16, remove_columns=['sentence'])
    ...
    #tokenized_train_dataset.save_to_disk("/mnt/storage/sarthak.joshi/tokenized_cola_train")
    #tokenized_val_dataset.save_to_disk("/mnt/storage/sarthak.joshi/tokenized_cola_validation")
    #tokenized_test_dataset.save_to_disk("/mnt/storage/sarthak.joshi/tokenized_cola_test")
    tokenized_train_dataset = load_from_disk("/mnt/storage/sarthak.joshi/tokenized_cola_train")
    tokenized_test_dataset = load_from_disk("/mnt/storage/sarthak.joshi/tokenized_cola_validation")
    tokenized_train_dataset = COLATokenizedDataset(tokenized_train_dataset)
    tokenized_test_dataset = COLATokenizedDataset(tokenized_test_dataset)
    
    trainer = Trainer(config,model,optimizer,tokenizer=tokenizer,train_dataset=tokenized_train_dataset,test_dataset=tokenized_test_dataset, seed=seed)
